...
| exception ledger | `.codestable/features/2026-07-12-mobile-mcp-black-box-contract-parity/mobile-mcp-black-box-contract-parity-exceptions.yaml` 中的具体外部行为豁免 | 当前有两项 `pending` 提案；未批准前 implementation/acceptance 不能把它们算 pass |
...
### Exception ledger proposal（需用户在本 design checkpoint 拍板）
...
若用户拒绝任一 exception，则对应真实成功行为成为 blocking implementation scope；不得退回 JSON `unsupported_platform`。
